...
const MatchDetails = (props) => {
    ...
    const [advanceStat,setAdvanceStat]=useState([]);
    useEffect(() => {
        matchBasicDetails();
    ...
    const matchAdvanceState = () =>{

        const payload = new FormData();
        payload.append('match_id', matchId);
        axios.post('/StageGoalyApi/matchStatsDetails', payload)
            .then(res => {
               
                if (res.data.success == 1) {

                    //console.log('============='+JSON.stringify(res.data.match_statsdetails))
                    setAdvanceStat(res.data.match_statsdetails);
                    setNoDataMatchInfo(false)
                }  
                if (res.data.success == 0) {
                    setAdvanceStat([]);
                    setNoDataMatchInfo(true)
    ...
    return (
        <div>
            ...
            <div className="row detail-match" style={{ paddingTop: " 6px" }}>
                ...
                {/*<div onClick={() => props.history.push(`/league/${matchInfo.sportsmonks_id}`)}>
                    <h4>{matchInfo.league_name}</h4>
                </div> */} 

                <Link
                            to={{
                                pathname: `/league/${matchInfo.sportsmonks_id}`,
                                state: { leagueId: matchInfo.sportsmonks_id,compId:matchInfo.league_id}
                              }}
                    ...
                    <div className="row">
                        ...
                        onClick={() =>props.history.push(`/club-info/${matchInfo.home_team_id}`)}>
                        
                            <img src={matchInfo.home_team_logo ? matchInfo.home_team_logo : imgMissingLogo} alt=""
                            />
                            <p>{matchInfo.home_team_name}</p>
                        </div>
                                    ...
                                    <div className="col-xs-5 d-flex ais-center j-start">
                                        ...
                                        <tbody>
                                            {(homeTeamGoals.length > 0) && homeTeamGoals.map((goal,key) => (
                                                <tr key={key}><td>{goal.player_name}-</td><td className="pl-3">{goal.minute}'</td></tr>
                                            ))}
                                            </tbody>
                                        </table>
                    ...
                    <div className="row row-no-gutters">
                        ...
                        <div className="col-xs-6 referee">
                            Referee: <br /><div style={{ textTransform: 'uppercase' }}>{matchInfo.referee}</div>
                        </div>
                    </div>
            ...
            {!!keys(matchInfo).length && <MatchTabs comment={comment} lineup={lineup}
             ...
             playersStats={playerStat} advancesStat={advanceStat} loadingPlayerStat={loadingPlayerStat}
