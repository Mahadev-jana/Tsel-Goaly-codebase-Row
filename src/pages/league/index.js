...
const League = (props) => {
    ...
    const statsDetails = () => {
        ...
        axios.post('/StageGoalyApi/leagueStats', payload)
            .then(res => {
                 console.log(res);
                 console.log("leagueStats api is hit");
                 if (res.data && res.data.code === 200 && res.data.success === 1 && res.data.error === 0) {
                    ...
                    let shortontotal=res.data.stats.filter(stat=>{
                           if(stat.title==="Short On Total" || stat.title==="Short On  Total"){
                               return true;
                               console.log("Short on total is fixed")
                           }
                           return false;
                    })
    ...
    const leaguesMainInfo = <div className="inner-league-cover">
        <div className="logo">
            <img src={BASE_URL + mainInfo.image} alt="" />
