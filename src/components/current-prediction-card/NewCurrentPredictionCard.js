...
const NewCurrentPredictionCard = React.memo(
  ({
    venue_image,
    isLoading,
    awayteam,
    ...
    function playContest() {
      ...
      if (getUserDetails() == null) {
        ...
        if (getUserDetails().balance_status === "insuflt_balance") {
          console.log("user login");
          console.log(getUserDetails().balance_status);
          Swal.fire({
            title:
              "You are not eligible to Join Competition since you have insufficient balance, please topup your balance and retry tomorrow",
            type: "info",
            confirmButtonText: "OKAY"
        })
        } else {
    ...
    const [{ open, predictionId }, setModal] = useState({
      open: false,
      predictionId: null,
    });
    ...
    return (
      <React.Fragment>
        <Modal
          open={open}
          onClose={() => setModal({ open: false, tab: "" })}
          center
          styles={{
            modal: {
              borderRadius: "5px",
        ...
        >
          {!isNull(id) && (
            <UserPlayContest
              ...
              closeModal={() => setModal({ open: false, predictionId: null })}
            />
          )}
        </Modal>
        {!isLoading && type == "news" ? (
          <div class="timeline-group">
            ...
            >
              <span class="float-left" style={{ textTransform: "capitalize" }}>
                <span class="icon-group mr-1"></span>{" "}
                <strong>{news} News</strong>
              </span>
          ...
          >
            <div class="timeline-match-header row min-gutter d-flex ais-center">
              ...
              {currentDate >= pred_start && currentDate <= pred_end  && (
                <div class="col-xs-4">
                  <span class="timeline-match-label blue">
                    Ongoing Prediction
                  </span>
                </div>
              )}
            ...
            <ul class="timeline-match-team">
              <li>
                <span
                  class="team-img"
                  onClick={() => history.push(`/club-info/${hometeamid}`)}
                >
                  <img src={hometeamlogo} alt="Team" />
                </span>
              ...
              <li>
                <span
                  class="team-img"
                  onClick={() => history.push(`/club-info/${awayteamid}`)}
                >
                  <img src={awayteamlogo} alt="Team" />
                </span>
            ...
            {currentDate >= pred_start && currentDate <= pred_end && (
              <div class="text-center" onClick={playContest}>
                <div class="btn bg-green p-1 w-50 my-1 text-white mx-auto">
                  Join
                </div>
              </div>
            )}
            {currentDate > pred_start && currentDate > pred_end && (
              <div class="text-center">
                <div
                  class="btn btn-purple p-1 w-50 my-1 text-white mx-auto"
                  onClick={() => history.push(`match/details/${match_id}`)}
                  style={{ padding: "5px" }}
                >
                  Result
                </div>
